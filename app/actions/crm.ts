    ...
    await db.transaction(async (tx) => {
      ...
      await tx.insert(schema.clients).values({
        ...
        details: JSON.stringify({
          ...
          websiteUrl: input.data.websiteUrl || (formData.get("domain") as string)?.trim() || "",
          industry: input.data.industry,
          country: input.data.country,
          services: input.data.services,
          primaryService: (formData.get("primaryService") as string) || input.data.services || "web_dev",
          adAccountId: (formData.get("adAccountId") as string)?.trim() || "",
          businessManagerId: (formData.get("businessManagerId") as string)?.trim() || "",
          instagram: (formData.get("instagram") as string)?.trim() || "",
          targetCpl: (formData.get("targetCpl") as string)?.trim() || "",
          targetRoas: (formData.get("targetRoas") as string)?.trim() || "",
          domain: (formData.get("domain") as string)?.trim() || input.data.websiteUrl || "",
          techStack: (formData.get("techStack") as string)?.trim() || "",
          hostingProvider: (formData.get("hostingProvider") as string)?.trim() || "",
          domainExpiry: (formData.get("domainExpiry") as string)?.trim() || "",
          cmsAccessUrl: (formData.get("cmsAccessUrl") as string)?.trim() || "",
          monthlyFee: (formData.get("monthlyFee") as string)?.trim() || "",
          contractDuration: (formData.get("contractDuration") as string)?.trim() || "",
