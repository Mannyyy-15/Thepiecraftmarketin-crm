...
import {
  ...
  updateInvoiceStatus, deleteProject,
} from "@/app/actions/crm";
import {
  ArrowLeft, Building2, CalendarDays, CheckCircle2, CheckSquare, Clock,
  Code2, DollarSign, Edit2, ExternalLink, FileText, Globe, Globe2,
  Layers, Link as LinkIcon, Loader2, Megaphone, MoreHorizontal,
  Phone, Plus, Receipt, Square, Tag, Trash2, TrendingUp, Users,
  X, Zap, AlertCircle, BadgeCheck, Database, Cpu, Server,
  LayoutGrid, ChevronRight, Target, BarChart3, SlidersHorizontal,
  ListTodo, Printer,
} from "lucide-react";
...
export default function ProjectDetailPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [pr, tr] = await Promise.all([getProjectById(Number(id)), getTeamUsers()]);
      if (pr.success && pr.data) setProject(pr.data);
  ...
  const handleAddTask = async () => {
    ...
    setAddingTask(true);
    try {
      const r = await addProjectTask(project.id, newTask.trim(), newPriority);
  ...
  const handleDelete = async () => {
    ...
    if (r.success) { toast(`${project.name} deleted.`, "info"); router.push("/admin/projects"); }
    else toast(r.error || "Failed.", "error");
  };

  const openEdit = () => {
    setEditForm({
      name: project.name, clientName: project.clientName || "", status: project.status,
      priority: project.priority, leadId: project.leadId ? String(project.leadId) : "",
      startDate: project.startDate || "", deadline: project.deadline || "",
      monthlyFee: project.monthlyFee || "", budget: project.budget || "",
...
<p style="margin-top:40px;color:#94a3b8;font-size:12px">Generated by ThePieCraft CRM · ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="h-6 w-6 border-2 border-slate-200 border-t-brand-500 rounded-full animate-spin" />
    </div>
  );
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      <div className="flex items-center gap-3">
        ...
        {project.clientName && (
          <>
            <button onClick={() => project.client && router.push(`/admin/clients/${project.client.id}`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <BentoCard className="lg:col-span-12 bg-gradient-to-r from-brand-500/5 via-violet-500/5 to-transparent">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">{PROJ_TYPE_LABEL[project.projectType] || "Project"}</p>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1.5">
                  ...
                  <span className={cn("inline-flex items-center text-[9px] font-bold px-2 py-0.5 rounded border", PRIORITY_PILL[project.priority] || PRIORITY_PILL.medium)}>
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline" onClick={printDocument} title="Download Proposal / SOW as PDF">
                <FileText className="h-3.5 w-3.5 mr-1.5" />Export PDF
              </Button>
              <Button size="sm" variant="outline" onClick={openEdit}>
                <Edit2 className="h-3.5 w-3.5 mr-1.5" />Edit Project
              </Button>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-6">
            {project.lead && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Lead Developer</p>
            ...
            {project.deadline && (
              <div>
                ...
                <p className={cn("text-xs font-bold", daysLeft !== null && daysLeft < 7 && daysLeft >= 0 ? "text-rose-500" : "text-slate-700 dark:text-slate-200")}>
                  {project.deadline} {daysLeft !== null && <span className="text-slate-400 font-normal">({daysLeft > 0 ? `${daysLeft}d left` : daysLeft === 0 ? "due today" : "overdue"})</span>}
                </p>
              </div>
            )}
        ...
        {/* ── KPI mini cards ───────────────────────────────────────────────── */}
        {[
          { label: "Budget / Fee", value: fee || "—", sub: project.billingModel?.replace(/_/g, " ") || "—", icon: DollarSign, color: "text-slate-800 dark:text-white", bg: "" },
          { label: "Tasks Done", value: `${tasksDone} / ${tasks.length}`, sub: `${tasksPct}% complete`, icon: CheckCircle2, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50/30 dark:bg-emerald-950/10" },
          { label: "Days Remaining", value: daysLeft !== null ? (daysLeft < 0 ? "Overdue" : `${daysLeft}d`) : "—", sub: project.deadline || "No deadline set", icon: CalendarDays, color: daysLeft !== null && daysLeft < 7 ? "text-rose-500" : "text-slate-800 dark:text-white", bg: daysLeft !== null && daysLeft < 7 ? "bg-rose-50/40 dark:bg-rose-950/10" : "" },
          { label: "Ad Spend Budget", value: project.adSpendBudget ? `$${project.adSpendBudget.toLocaleString()}/mo` : "—", sub: "monthly media budget", icon: Target, color: "text-brand-600 dark:text-brand-400", bg: "bg-brand-50/30 dark:bg-brand-950/10" },
        ].map(kpi => (
          <BentoCard key={kpi.label} className={cn("lg:col-span-3", kpi.bg)}>
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1 capitalize">{kpi.sub}</p>
              </div>
        ...
        <BentoCard className="lg:col-span-8">
          ...
          <div className="flex gap-2 mb-4">
            <input value={newTask} onChange={e => setNewTask(e.target.value)}
              ...
              placeholder="Add a task… (press Enter)" className={cn(INPUT, "flex-1 h-9 text-xs")} />
            <select value={newPriority} onChange={e => setNewPriority(e.target.value)} className={cn(SELECT, "w-28 h-9 text-xs")}>
              <option value="high">High</option>
              <option value="medium">Medium</option>
              <option value="low">Low</option>
            </select>
            <Button size="sm" onClick={handleAddTask} disabled={addingTask || !newTask.trim()} className="bg-brand-600 text-white shrink-0">
              {addingTask ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />}
            </Button>
          </div>
          {/* Task list */}
          {!tasks.length ? (
            <EmptyState icon={<ListTodo className="h-4 w-4" />} title="No tasks yet" description="Add tasks above to track project progress." />
          ) : (
              ...
              {tasks.map((t: any) => (
                <div key={t.id}
                  className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all",
                    t.done === 1
                      ? "border-slate-100 dark:border-slate-800/60 bg-slate-50/40 dark:bg-slate-900/20"
                      : "border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900/40"
                  )}>
                  <button onClick={() => handleToggleTask(t.id, t.done)} disabled={toggling === t.id}
                  ...
                  <span className={cn("flex-1 text-xs font-medium", t.done === 1 && "line-through text-slate-400")}>{t.title}</span>
                  <span className={cn("text-[9px] font-bold px-1.5 py-0.5 rounded border shrink-0", TASK_PRIORITY_PILL[t.priority] || TASK_PRIORITY_PILL.medium)}>
                    {t.priority}
                  </span>
                  {t.dueDate && <span className="text-[9px] text-slate-400 shrink-0 hidden sm:block">{t.dueDate}</span>}
                  <button onClick={() => handleDeleteTask(t.id)}
                    className="shrink-0 h-6 w-6 rounded-lg text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 flex items-center justify-center transition-all cursor-pointer">
        ...
        {/* ── Project Details ──────────────────────────────────────────────── */}
        <BentoCard className="lg:col-span-4 space-y-4">
          <SectionTitle icon={SlidersHorizontal} label="Details" />
          <div className="space-y-3">
            {[
              ...
              { label: "Access Granted",     value: project.accessGranted ? "Yes" : "No" },
            ].filter(r => r.value).map(r => (
              <div key={r.label} className="flex items-center justify-between gap-3 py-2 border-b border-slate-100 dark:border-slate-800/60 last:border-0">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{r.label}</span>
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200 text-right capitalize">{r.value}</span>
              </div>
            ))}
          </div>
          {/* Service details */}
          ...
          {project.contractLink && (
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
              <a href={project.contractLink} target="_blank" rel="noreferrer"
                ...
                <LinkIcon className="h-3.5 w-3.5" />View Contract / SOW
              </a>
            </div>
          )}
          {/* Old website link for web_dev */}
          {project.projectType === "web_dev" && sd.oldWebsiteUrl && (
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
              <a href={sd.oldWebsiteUrl.startsWith("http") ? sd.oldWebsiteUrl : `https://${sd.oldWebsiteUrl}`}
                ...
                <Globe2 className="h-3.5 w-3.5" />Current / Old Website
              </a>
            </div>
          )}
        </BentoCard>
          ...
          <div className="space-y-3">
            {project.client && (
              <button onClick={() => router.push(`/admin/clients/${project.client.id}`)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-400">View client →</p>
                </div>
            ...
            {project.clientContactName && (
              <div className="space-y-1.5">
                ...
                {project.clientContactPhone && (
                  <div className="flex items-center gap-2">
                    <a href={`tel:${project.clientContactPhone}`} className="text-xs text-slate-500 hover:underline">{project.clientContactPhone}</a>
                    <a href={`https://wa.me/${project.clientContactPhone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer"
                      className="inline-flex items-center gap-1 text-[10px] font-bold text-white bg-emerald-500 hover:bg-emerald-600 px-2 py-0.5 rounded-full transition-all">
                      WA
                    </a>
                  </div>
                )}
        ...
        <BentoCard className="lg:col-span-8">
          ...
          {!invs.length ? (
            <EmptyState icon={<Receipt className="h-4 w-4" />} title="No invoices" description="No invoices linked to this project yet." />
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/30">
                    ...
                    <th className="text-left px-4 py-2.5 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Due</th>
                  ...
                  {invs.map((inv: any) => {
                    ...
                    return (
                      <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                        <td className="px-4 py-3 font-mono text-[11px] font-bold text-slate-600 dark:text-slate-300">{inv.invoiceNumber}</td>
                        ...
                        <td className="px-4 py-3 text-slate-500 whitespace-nowrap">
                          {inv.paidDate ? <span className="text-emerald-600 font-semibold">Paid {inv.paidDate}</span>
                            : inv.dueDate ? <span className={cn(isOverdue && "text-rose-500 font-bold")}>{inv.dueDate}</span> : "—"}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          {(inv.status === "sent" || inv.status === "overdue") && (
                            <button onClick={() => handleMarkPaid(inv.id)}
                              className="text-[9px] font-bold text-emerald-600 hover:underline cursor-pointer whitespace-nowrap">
                              Mark paid
                            </button>
                          )}
                        </td>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-[540px] bg-white dark:bg-slate-950 z-50 shadow-2xl flex flex-col animate-[slide-in-right_280ms_cubic-bezier(0.16,1,0.3,1)]">
            <div className="shrink-0 px-6 pt-5 pb-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="text-base font-bold text-slate-900 dark:text-white mt-0.5">{project.name}</h2>
                </div>
            ...
            <form onSubmit={handleSaveEdit} className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                <div><label className={LABEL}>Project Name *</label><input required value={editForm.name || ""} onChange={e => setEditForm((p: any) => ({ ...p, name: e.target.value }))} className={INPUT} /></div>
                <div><label className={LABEL}>Status</label>
                  <select value={editForm.status || "planning"} onChange={e => setEditForm((p: any) => ({ ...p, status: e.target.value }))} className={SELECT}>
                    {["planning","active","paused","completed","cancelled"].map(s => <option key={s} value={s}>{STATUS_CONFIG[s]?.label || s}</option>)}
                  </select>
                </div>
                <div><label className={LABEL}>Priority</label>
                  <select value={editForm.priority || "medium"} onChange={e => setEditForm((p: any) => ({ ...p, priority: e.target.value }))} className={SELECT}>
                    <option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option>
                  </select>
                </div>
                <div><label className={LABEL}>Lead Developer</label>
                  <select value={editForm.leadId || ""} onChange={e => setEditForm((p: any) => ({ ...p, leadId: e.target.value }))} className={SELECT}>
                    <option value="">Unassigned</option>
                    {roster.map((u: any) => <option key={u.id} value={u.id}>{u.name}</option>)}
                  </select>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={LABEL}>Start Date</label><input type="date" value={editForm.startDate || ""} onChange={e => setEditForm((p: any) => ({ ...p, startDate: e.target.value }))} className={INPUT} /></div>
                  <div><label className={LABEL}>Deadline</label><input type="date" value={editForm.deadline || ""} onChange={e => setEditForm((p: any) => ({ ...p, deadline: e.target.value }))} className={INPUT} /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={LABEL}>Monthly Fee ($)</label><input type="number" value={editForm.monthlyFee || ""} onChange={e => setEditForm((p: any) => ({ ...p, monthlyFee: e.target.value }))} className={INPUT} /></div>
                  <div><label className={LABEL}>Project Budget ($)</label><input type="number" value={editForm.budget || ""} onChange={e => setEditForm((p: any) => ({ ...p, budget: e.target.value }))} className={INPUT} /></div>
                </div>
                <div><label className={LABEL}>Ad Spend Budget ($/mo)</label><input type="number" value={editForm.adSpendBudget || ""} onChange={e => setEditForm((p: any) => ({ ...p, adSpendBudget: e.target.value }))} className={INPUT} /></div>
                <div><label className={LABEL}>Contract Duration (months)</label><input type="number" value={editForm.contractDuration || ""} onChange={e => setEditForm((p: any) => ({ ...p, contractDuration: e.target.value }))} className={INPUT} /></div>
                <div><label className={LABEL}>Client Contact Name</label><input value={editForm.clientContactName || ""} onChange={e => setEditForm((p: any) => ({ ...p, clientContactName: e.target.value }))} className={INPUT} /></div>
                <div><label className={LABEL}>Client Contact Phone</label><input value={editForm.clientContactPhone || ""} onChange={e => setEditForm((p: any) => ({ ...p, clientContactPhone: e.target.value }))} className={INPUT} /></div>
                <div><label className={LABEL}>Contract / SOW Link</label><input value={editForm.contractLink || ""} onChange={e => setEditForm((p: any) => ({ ...p, contractLink: e.target.value }))} className={INPUT} /></div>
              </div>
              <div className="shrink-0 px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-900/20 flex justify-end gap-3">
                ...
                <Button type="submit" size="sm" disabled={editSaving} className="bg-brand-600 text-white font-bold min-w-[120px] justify-center">
