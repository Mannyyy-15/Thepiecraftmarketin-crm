      ...
      <div className="px-4 pt-4 pb-2 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-10 rounded-2xl border border-slate-200 dark:border-[#38383f] bg-slate-50 dark:bg-[#1f1f1f] pl-9 pr-4 text-xs focus:outline-none focus:ring-2 focus:ring-brand-500/40"
          />
        </div>
        <button
          onClick={() => { setShowContactPicker(true); setPickerSearch(""); }}
          className="h-10 w-10 rounded-2xl bg-brand-500 text-white flex items-center justify-center hover:bg-brand-600 transition-colors shrink-0 cursor-pointer"
          ...
          <>
              ...
              {messages.map((m: any) => {
                ...
                return (
                  <div key={m.id} className={`flex ${isMine ? "justify-start" : "justify-end"}`}>
                    <div className={`max-w-[80%] rounded-[20px] px-4 py-2.5 ${isMine ? "bg-slate-100 dark:bg-[#303030] rounded-bl-sm" : "bg-brand-500 text-white rounded-br-sm"}`}>
            ...
            <div className="border-t border-slate-100 dark:border-[#303030]/80 p-4 shrink-0">
              <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-2">
                <input type="text" value={newMsg} onChange={(e) => setNewMsg(e.target.value)} placeholder="Type a message..." className="flex-1 h-11 rounded-2xl border border-slate-200 dark:border-[#38383f] bg-slate-50 dark:bg-[#1f1f1f] px-4 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/40" />
                <button type="submit" className="h-11 w-11 rounded-2xl bg-brand-500 text-white flex items-center justify-center hover:bg-brand-600 transition-colors cursor-pointer disabled:opacity-50" disabled={!newMsg.trim()}>
        ...
        <div className="fixed inset-0 z-50 bg-black/40" onClick={() => setShowContactPicker(false)}>
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-[#1f1f1f] rounded-t-2xl p-4 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative mb-3">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
              <input type="text" value={pickerSearch} onChange={(e) => setPickerSearch(e.target.value)} placeholder="Search contacts..." className="w-full h-10 rounded-2xl border border-slate-200 dark:border-[#38383f] bg-slate-50 dark:bg-[#1f1f1f] pl-9 pr-4 text-xs focus:outline-none" />
            </div>
