...
import { documents as mockDocuments } from "@/lib/mock";
...
export default function DocumentsPage() {
  ...
  const handleCreateFolder = (e: React.FormEvent) => {
    ...
    addToast(`Successfully created folder "${newFolder.name}"!`);
  };
  ...
  const handleUploadFile = (e: React.FormEvent) => {
    ...
    // Update folder file count matching the mock categories (heuristic mapping)
    ...
    if (formattedName.toLowerCase().includes("contract")) folderTarget = "Contracts";

    setFolders(prev => prev.map(f => {
      if (f.name === folderTarget) {
        return {
          ...f,
          files: f.files + 1,
          size: "Recalculated"
      ...
      <Card className="overflow-hidden border border-slate-200 dark:border-slate-850">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20">
          ...
          <div className="relative max-w-sm sm:w-72">
            <Search className="pointer-events-none absolute inset-y-0 left-3 h-full w-4 text-slate-400 animate-pulse" />
            <input
              type="search"
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredFiles.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-900/40 group">
                  ...
                  <td className="px-5 py-3.5 hidden md:table-cell text-slate-600 dark:text-slate-300 font-medium">{d.client}</td>
                  ...
                  <td className="px-5 py-3.5 hidden sm:table-cell text-slate-500 dark:text-slate-400 text-xs">{d.updated}</td>
                  <td className="px-5 py-3.5 tabular-nums text-slate-600 dark:text-slate-300 font-medium text-xs">{d.size}</td>
          ...
          <Card className="w-full max-w-md animate-scaleIn border border-indigo-500/25 shadow-2xl">
            <CardHeader className="py-4 border-b dark:border-slate-800">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowUploadModal(false)} className="text-slate-400 hover:text-slate-650">
                  <X className="h-4.5 w-4.5" />
                </button>
              </div>
            </CardHeader>
          ...
          <div
            ...
            className={`pointer-events-auto p-4 rounded-xl shadow-xl flex items-center justify-between gap-3 border transition-all animate-slideIn ${
              t.type === "warning"
                ...
                : t.type === "info"
                ? "bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-350"
