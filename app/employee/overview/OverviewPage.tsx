...
function computeTodayHours(attendance: any[], timesheets: any[], user: any) {
  const todayStr = toLocalDateStr(new Date());
  ...
  return Number(todayHoursVal.toFixed(1));
}
...
export default function OverviewPage({ initialData }: OverviewPageProps) {
  const [isLoading, setIsLoading] = useState<boolean>(!initialData);
  const [user, setUser] = useState<any>(initialData?.user ?? null);
  const [employeeProjects, setEmployeeProjects] = useState<any[]>(initialData?.projects ?? []);
  const [allTimesheets, setAllTimesheets] = useState<any[]>(initialData?.timesheets ?? []);
  const [allAttendance, setAllAttendance] = useState<any[]>(initialData?.attendance ?? []);
  const [weeklyHours, setWeeklyHours] = useState<any[]>([]);
  const [totalHoursToday, setTotalHoursToday] = useState<number>(() =>
    initialData ? computeTodayHours(initialData.attendance ?? [], initialData.timesheets ?? [], initialData.user ?? null) : 0
  );
  ...
  const [tasks, setTasks] = useState<Task[]>(() =>
    initialData
      ? initialData.tasks.map((t: any) => {
          const matchedProj = (initialData.projects ?? []).find((p: any) => p.id === t.projectId);
          return {
            id: t.id,
            title: t.title,
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setTotalHoursToday(computeTodayHours(attendance, timesheets, user));
    } catch (err) {
