  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-[#303030] dark:bg-[#1f1f1f]">
        ...
        {secret && (
          <div className="mt-5 space-y-3 rounded-xl bg-slate-50 p-4 dark:bg-[#303030]">
            <p className="text-sm">Manual setup key</p>
            <code className="block break-all rounded-lg bg-white p-3 text-sm dark:bg-[#1f1f1f]">{secret}</code>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <input aria-label="Authenticator code" inputMode="numeric" autoComplete="one-time-code" value={code} onChange={(event) => setCode(event.target.value)} placeholder="6-digit code" className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 py-2 dark:border-[#303030] dark:bg-[#1f1f1f]" />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-[#303030] dark:bg-[#1f1f1f]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button disabled={pending} onClick={() => startTransition(async () => { await revokeOtherDeviceSessions(); await refreshSessions(); })} className="rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold dark:border-[#303030]">
            Sign out other devices
          </button>
        </div>
        <div className="mt-4 divide-y divide-slate-200 dark:divide-[#303030]">
          {sessions.map((session) => (
            <div key={session.sessionId} className="flex items-center gap-3 py-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">{session.deviceName || "Unknown device"} {session.current && <span className="text-emerald-600">Â· This device</span>}</p>
