  ...
  return (
    <main className="space-y-6 pb-12">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <select aria-label="Organization" value={organizationId} onChange={(event) => { const id = Number(event.target.value); setOrganizationId(id); void loadRows(id); }} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950">
      ...
      <form action={submit} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-4 dark:border-slate-800 dark:bg-slate-950">
        <input required name="name" maxLength={255} placeholder={kind === "contacts" ? "First name" : `${copy[kind].title.slice(0, -1)} name`} className="rounded-xl border border-slate-300 bg-transparent px-3 py-2 text-sm dark:border-slate-700" />
        {kind === "accounts" && <input name="website" type="url" placeholder="https://company.com" className="rounded-xl border border-slate-300 bg-transparent px-3 py-2 text-sm dark:border-slate-700" />}
        {kind === "contacts" && <>
          <input name="lastName" placeholder="Last name" className="rounded-xl border border-slate-300 bg-transparent px-3 py-2 text-sm dark:border-slate-700" />
          <input name="email" type="email" placeholder="Email" className="rounded-xl border border-slate-300 bg-transparent px-3 py-2 text-sm dark:border-slate-700" />
        </>}
        {kind === "deals" && <input name="amount" type="number" min="0" step="1" placeholder="Value (INR)" className="rounded-xl border border-slate-300 bg-transparent px-3 py-2 text-sm dark:border-slate-700" />}
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {rows.map((row) => (
            <article key={row.id} className="flex flex-col justify-between gap-2 p-4 sm:flex-row sm:items-center">
              ...
              {row.status && <span className="w-fit rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium dark:bg-slate-800">{row.status}</span>}
            </article>
          ))}
